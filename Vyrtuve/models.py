...
class Profilis(models.Model):
    ...
    def update_rating(self):
        ...
        total_rating = sum([rating.reitingas for rating in self.reitingai.all()])
        total_ratings = self.reitingai.count()

        if total_ratings > 0:
            self.vidutinis_reitingas = total_rating / total_ratings
        else:
...
class Receptas(models.Model):
    ...
    def start_counting(self):
        ...
        reitingai = self.reitingai.all()

        self.reitingu_kiekis = reitingai.count()

        total_rating = 0

        for rating in reitingai:
        ...
        self.favoritu_kiekis = reitingai.filter(favoritas=True).count()
